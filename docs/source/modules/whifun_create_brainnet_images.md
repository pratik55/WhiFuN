...
This function iterates through all unique network labels in the input ROI NIfTI file and creates a visualization of each network from multiple standard viewing angles using BrainNet Viewer.

## Input Arguments

### `OUT_PATH`
The directory where the resulting PNG images will be saved.

### `ROI_PATH`
The full path to the input NIfTI file containing the labeled Functional Network (FN) map (e.g., 'WM_clustering_K10.nii').

### `OVER_WRITE`
Flag (0 or 1). If 0, image generation will be skipped if the first output file already exists. Output: A set of PNG files saved to OUT_PATH, named in the format: `<ROI_name>_<Network_Label>_<View_Name>.png`
