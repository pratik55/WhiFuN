...
configures and executes the SPM realignment job to correct for head motion in a time series of functional images.
...
### `now_func_path`
A `dir` structure pointing to the functional NIfTI file.

### `Realign_pre`
The prefix to use for the output realigned file (e.g., 'r').
