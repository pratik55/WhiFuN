...
**Source:** `whifun_functions/whifun_create_FN_Kmeans.m:1`
...
average voxel-level functional connectivity matrix across subjects and generates functional networks (FNs). It also saves BrainNet images and average time series for each FN.

Required Inputs: output_folder    - Path to output folder WM_or_GM         - 'WM' or 'GM' K_range_l        - Lower bound for K (clusters) K_range_h        - Upper bound for K group_mask_path  - Path to group-level brain mask (NIfTI)
...
Example: 'WM_or_GM','WM','K_range_l',2,'K_range_h',10,... 'group_mask_path','mask.nii');
