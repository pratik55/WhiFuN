...
This function checks if previously clustered FN files exist in the specified folder. If files are found, it prompts the user to either rebuild the networks or select an existing cluster solution (K value) to proceed with.
...
### `OUT_PATTERN`
The filename pattern used for the clustered NIfTI files (e.g., 'WM_FN_K*.nii', where * is the K value).

### `OVER_WRITE`
(Optional, default 0) Flag to bypass prompts if set to 1.
...
'whifun_create_file', 'questdlg', 'inputdlg' functions.
